...
def plot_gradients(Potential_grid_mgrid,NGX,NGY,NGZ):
...
#   Get the data in the correct plane
    grad_x = np.zeros(shape=(NGX,NGY))
    grad_y = np.zeros(shape=(NGX,NGY))
    grad_z = np.zeros(shape=(NGX,NGY))
    CutPlane = float(raw_input("Fraction along the z-axis where you wish to cut?  "))
    grad_x[:,:] = grad_x_1[:,:,NGZ*CutPlane]
    grad_y[:,:] = grad_y_1[:,:,CutPlane*NGZ]
    grad_z[:,:] = grad_z_1[:,:,CutPlane*NGZ]
# Plotting   
#    quiver(mg_x[::3],mg_y[::3],grad_x[::3],grad_y[::3],
#         color='r', angles='xy',scale=0.0001)
#    show()
...
def Planar_Plotting(NGX,NGY,NGZ,Potential,Lattice):
 ...
 # Now get the macroscopic average
     ...
     return(Macro_Potential,Plane_Potential_New, Centre_potential)
...
average_type=raw_input("Which kind of average would you like? (P)lanar/(S)pherical/(Po)int/(G)radients spherical average ")

#3 Convert the data to a more friendly format

if average_type != "P":
 Potential_grid_mgrid = list_2_matrix(Potential,NGX,NGY,NGZ)
 Potential_grid = grid_2_xyz(Potential_grid_mgrid,lattice)
if average_type != "Po":
...
if average_type == 'P':
   Macro_Potential,Plane_Potential_New,Centre_potential = Planar_Plotting(NGX,NGY,NGZ,Potential,lattice)
...
# Generate gradients and mesh
elif average_type == 'G':
   plot_gradients(Potential_grid_mgrid,NGX,NGY,NGZ)
